...
interface StripeDepositCheckoutProps {
...
const CheckoutForm = ({ amount, onSuccess, onError }: StripeDepositCheckoutProps) => {
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
            ...
            >
                {isLoading ? (
                    ...
                    `Pay â‚¹${amount.toLocaleString('en-IN', { minimumFractionDigits: 2 })} with Stripe`
                )}
            </button>
        </form>
    );
...
export const StripeDepositCheckout = (props: StripeDepositCheckoutProps) => {
    ...
    useEffect(() => {
        const fetchClientSecret = async () => {
            try {
                const response = await axiosClient.post(`/payments/stripe/create-deposit-intent/${props.appointmentId}`);
            ...
            {clientSecret ? (
                ...
                >
                    {isMockLoading ? (
                        ...
                        `Pay â‚¹${props.amount.toLocaleString('en-IN', { minimumFractionDigits: 2 })} with Stripe`
                    )}
                </button>
            ) : (
